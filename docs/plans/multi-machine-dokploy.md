...
- **Release B: hosting move to Dokploy.** Linux packaging, backups, isolated pilot, and cutover against a client contract that Release A already proved. Multi-source T3 support (ST-160) lands here: as of 2026-09-17 only the Mac runs T3 Code, so Release A did not need it.

Release A closed on 2026-09-17 with two machines (the Mac mini host and agent-ubuntu) reporting through the central API. Kevin deferred the third machine; onboarding it later is a repeat of the ST-161 installer flow plus a credential, not new work.
...
Decision (Kevin, 2026-09-13): all agent machines share one local network with the Factory host, and they address each other by local IP. Inventory so far: the Factory host is the Mac mini (`mac-mini`) at `192.168.6.100`, serving `http://192.168.6.100:3000`; its own machine credential `mac-mini` is scoped to all six Projects. ST-166 was deployed to the Mac LaunchAgent and verified by Kevin on 2026-09-13. Second machine: `agent-ubuntu` (Ubuntu 26.04.1 LTS, x86_64) at `192.168.6.51`, SSH alias `ubuntu-agent`; onboarded 2026-09-14 with the linux-x64 client, credential `agent-ubuntu` scoped to all six Projects, doctor passing over the LAN, and a report attributed to `agent-ubuntu`. On 2026-09-18 T3 Code 0.0.42 was installed as a standalone user service, Kevin connected T3 Connect, and a separate read-only endpoint was verified at `http://192.168.6.51:3773`. On 2026-09-17 Kevin installed Codex 0.154.0, Claude and git and trusted the Codex hook; a fresh `codex exec` session there received the Forge brief through the installed binary. On 2026-09-17 Kevin set the Factory Project's Git origin and configured GitHub SSH on that machine (as `respectthebot`); a real clone of the T-37 branch at `~/Projects/factory` there yields the Factory brief naming T-37 in a live Codex session. The third machine is deferred (Kevin, 2026-09-17). T3 Code runs per machine, as a desktop application or standalone user service. Factory polls authenticated read-only endpoints centrally over the private LAN. Ubuntu listens on all interfaces so its loopback T3 Connect relay and LAN reader both work; its observed interfaces are private. The Mac reader currently remains on loopback while Factory is hosted there; making it reachable privately from Dokploy is a pilot prerequisite. The T3 reader currently rejects plain HTTP for any non-loopback host; ST-160 relaxes that for private (RFC 1918) addresses only, never for public hosts. The push relay alternative is not pursued.
...
Source configuration, workspace mappings, token renewal and migration rehearsal evidence
are recorded in [the multiple-source runbook](../t3-multiple-sources.md).
...
| ST-160 | B | Multiple T3 sources and workspace mappings | ST-159; a second machine that runs T3 | Duplicate thread IDs, same branch, independent sequences, unavailable source and migration preservation tests |
...
| ST-164 | B | Isolated pilot | Release A complete, ST-160, ST-162, ST-163 | Reporting from both onboarded machines (third deferred), denied verification, restart/reconnect, outage, revocation and restore demonstrations |
...
4. Point both onboarded clients and the mobile dashboard at the new endpoint (third machine remains deferred). Verify machine credentials, repository context, reports and source-specific associations.
